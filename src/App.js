...
export default function App() {
  ...
  const exportExcel = (b) => {
    const ws = XLSX.utils.json_to_sheet([{ Date: currentTime.toLocaleDateString(), Particulars: 'Opening Balance', Amount: b.balance }]);
  ...
  const exportPDF = (b) => {
    const doc = new jsPDF();
    doc.text(`Statement: ${b.bankName}`, 14, 15);
    doc.autoTable({
      startY: 25,
        ...
        <div className="content-area">
          {activeTab === "Dashboard" && (
            <div className="fade-in">
                      ...
                      {banks.filter(b => b.firmName === selectedFirm).map(b => (
                        <React.Fragment key={b.id}>
                          <tr>
                            ...
                            <td><button className="btn-gold-sm" onClick={() => setExpandedBank(expandedBank === b.id ? null : b.id)}>View Details</button></td>
                          </tr>
                          {expandedBank === b.id && (
                            <tr>
                              <td colSpan="4">
                                <div className="ledger-panel">
                                  <div className="flex-between mb-10">
                                    <span className="ledger-title">Transaction History</span>
              ...
              ) : <div className="empty-state-card">Please select a firm from the menu above to view reports.</div>}
            </div>
          )}
                  ...
                  <tbody>
                    {usersList.map(u => (
                      <tr key={u.id}>
                        ...
                        <td><button className="btn-del-sm" onClick={async () => {if(window.confirm("Delete User?")) await deleteDoc(doc(db, "users", u.id))}}>Delete</button></td>
                      </tr>
                    ))}
                  </tbody>
                </table>
    ...
    <div className="app-shell">
      ...
      <div className="main-stage">
        ...
        <div className="content-area">
            ...
            <div className="fade-in">
              <div className="card-premium">
                ...
                <form className="master-grid-form" onSubmit={async (e) => {
                  ...
                  <input name="uPhone" placeholder="Mobile" className="pro-input" required />
                  <select name="uRole" className="pro-input">
                    <option value="Admin">Admin</option>
                    <option value="Operator">Operator (Add Only)</option>
                    ...
                    {usersList.map(u => (
                      <tr key={u.id}>
                        ...
                        <td>
                          ...
                          <button className="btn-del-sm" onClick={async () => {if(window.confirm("Delete?")) await deleteDoc(doc(db, "users", u.id))}}>Delete</button>
                        </td>
                      </tr>
                    ))}
                  </tbody>
          ...
          {/* Firm & Bank Masters will follow the same pattern with Address & Branch fields */}
        </div>
      </div>
    </div>
  );
